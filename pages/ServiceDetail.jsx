...
export default function ServicePage() {
  ...
  useEffect(() => {
    let active = true;
    fetchServices().then((rows) => { if (active && rows.length) setServices(rows); }).catch(() => {});
    fetchProjects().then((rows) => { if (active && rows.length) setProjects(rows); }).catch(() => {});
    return () => { active = false; };
  }, []);
      ...
      <section className="case section section--padless">
        <div className="container case-gallery">
          <div className="cg-row cg-row--2">
            ...
            <a href={latest ? `/work/${latest.slug}` : "/work"} className="cg-item cg-item--latest reveal" data-cursor="view">
